...
if [[ -e ${TMPDIR:-/tmp}/stow-bin.sh.log ]]; then
  rm ${TMPDIR:-/tmp}/stow-bin.sh.log
fi

# Stow/unstow binaries
stow $stowcom -vt "$targetdir" "$arch" \
  2> >(tee -a ${TMPDIR:-/tmp}/stow-bin.sh.log >&2)

# Stow/unstow shell scripts
stow $stowcom -vt "$targetdir" scripts \
  2> >(tee -a ${TMPDIR:-/tmp}/stow-bin.sh.log >&2)
...
if [[ -n $unstow ]]; then
  ...
  mansubs=(*) && popd >/dev/null
    for r in "${mansubs[@]}"; do
      if [[ -d $mandir/$r ]]; then
        rmdir --ignore-fail-on-non-empty "${mandir}/${r}"; fi
    done
